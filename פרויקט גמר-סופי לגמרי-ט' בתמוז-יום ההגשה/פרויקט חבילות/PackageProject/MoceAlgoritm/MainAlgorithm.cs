...
namespace Algoritm
{
    ...
    {
        private readonly float RADIOUS = 10f;//מרחק הרדיוס של כל עובד
        private readonly int NUMOFOPTION = 1000;//מס' האופציות האפשריות שמגרילים
        ...
        public static double [,] DistansesCol { get; set; }//מטריצת מרחקים בין נקודות איסוף
            ...
            , List<PackageDto> lstPackages, IService<CollectingPointDto> collectingPoints)//פעולה בונה של האלגוריתם
        {
            ...
            //בינתיים אחר כך למחוק--------------------------------------------------------
            int ce = LstEmployees.Count - 3;
            
            for (int i = 0; i < ce; i++)
            {
                LstEmployees.RemoveAt(0);
            }
            //----------------------------------------------------------------------------
            //TODO 2:
        ...
        private async Task OpenBusinessDayAsync()
        {//לבדוק שבאמת מחכה להמתנה
            CurrentBusinessDay = (await _businessDays.AddAsync(new BusinessDayDto())).Last();
        }
        ...
        private void saveDistance()//שומרת מרחק -ע''י נוסחה חשבונית
        {
            ...
            DistansesCol = new double[max, max];
            for (int i = 0; i < LstCollectingPoints.Count; i++)
            {
        ...
        public static DistanceType googleMapsDistance(CollectingPointDto c1, CollectingPointDto c2)
        {
            ...
            return new DistanceType() { kilometer = DistansesCol[c1.indexDistance, c2.indexDistance]
        ...
        public void employeeInInitialCollectionPoint()
        {
            ...
            //לכל עובד מרנדמים בתוך הנקודות שבאזורו שיש שם חבילות נקודה כל שהיא
            {
                ...
                lstCollectingPointWithPackages = employee.colectingPointsOfEmployees.Where(c => c.Value.Packages.Count > 0).ToList();
        ...
        public void initInteration(DateTime open)
        {
            ...
            {
                colPoint.Packages = new List<PackageInCollectingPoint>();
            }
            foreach (var package in LstPackages)
            {
                package.CurrentLocation = package.Source;
                package.Source.Value.Packages.Add(new PackageInCollectingPoint( package,true));
        ...
        public double markOfInteration(/*Dictionary<EmployeeDto, OptionToEmployee> option*/)
        {
            ...
            foreach (var package in LstPackages)
            {
                double totalTime = DistansesCol[package.Source.Value.indexDistance, package.Destination.Value.indexDistance];                  //googleMapsDistance(package.Source.Value, package.Destination.Value);
                double timeRemainWay;
                if (package.Source.Value == package.CurrentLocation.Value)
                    timeRemainWay = totalTime;
                else
                    timeRemainWay = DistansesCol[package.CurrentLocation.Value.indexDistance, package.Destination.Value.indexDistance];      //googleMapsDistance(package.CurrentLocation.Value, package.Destination.Value);
        ...
        //----------------------------------------לא העתקתי את הקוד הזה לספר פרויקט-------------------------------------------------------------------------------
        //----------------------------------------לא העתקתי את הקוד הזה לספר פרויקט-------------------------------------------------------------------------------
        //----------------------------------------לא העתקתי את הקוד הזה לספר פרויקט-------------------------------------------------------------------------------
        //----------------------------------------לא העתקתי את הקוד הזה לספר פרויקט-------------------------------------------------------------------------------
        public async void initializationOfVertices()//אתחול נקודות איסוף וחבילות-יעד ומקור
        {
            ...
            foreach (PackageDto package in packages)//הוספה לכל חבילה שבמצב המתנה 2 נקודות 
            {
                ...
                //MessageBox.Show("befor");
                //הוספת הנקודות איסוף החדשות למסד נתונים
                Task t =   _collectingPoints.AddAsync(collectingPointS);
                t.Wait();
                
                Task t2= _collectingPoints.AddAsync(collectingPointD);
                t2.Wait();
            ...
            Task<List<CollectingPointDto>> task = _collectingPoints.GetAllAsync();
            task.Wait();
            LstCollectingPoints = task.Result;
            ...
            foreach (CollectingPointDto point in LstCollectingPoints)//מכניסה את כל הנקודות איסוף לגרף כקודקודים בגרף
            {
                ...
                point.Packages = new List<PackageInCollectingPoint>();//איתחול החבילות שיש בנקודה לריק
                Graph.AddNode(point);
            ...
            foreach (PackageDto package in LstPackages)
            {
                GraphNode<CollectingPointDto> s = Graph.Nodes.Where(n => n.Value.Address == package.AddressSource).First();//לא יודעת אם בטוח-אבל אני צריכה רק אחרי ששמים בגרף את הנקודות המעודכנות-רק אז לשים את החבילות במקומן
                GraphNode<CollectingPointDto> d = Graph.Nodes.Where(n => n.Value.Address == package.AddressDestination).First();//לא יודעת אם בטוח-אבל אני צריכה רק אחרי ששמים בגרף את הנקודות המעודכנות-רק אז לשים את החבילות במקומן
                ...
                s.Value.Packages.Add(new PackageInCollectingPoint(package, true));//הוספה לחבילות שנמצאות בנקודה מקור את החבילה הנוכחית
        ...
        //לא נראה לי שצריך)-------------עובד עם מזהה עובד ולא עם מספור אחר כמו מנקודה לנקודה-לא טוב!!!!------!!!לא טוב
        //public void  saveDistanceEmployeeToCol()//כאן לא סידרתי שלא יהיה לפי המזהה אלא לפי אינדקס מיוחד לזה
        //{
        //    int maxCol = LstCollectingPoints.Max(c => c.CollectingPointId) + 1;
        //    int maxEmp = LstEmployees.Max(c => c.EmployeeId) + 1;
        //    DistansesEmployeeToCol =new double[maxEmp ,maxCol];
        //    for (int i = 0; i < LstEmployees.Count; i++)
        //    {
        //        for (int j = 0; j < LstCollectingPoints.Count; j++)
        //        {
        //            DistansesEmployeeToCol[LstEmployees[i].EmployeeId, LstCollectingPoints[j].CollectingPointId] =
        //                DistanceBetweenPoints((double)LstEmployees[i].LocationX, (double)LstEmployees[i].LocationY, (double)LstCollectingPoints[j].LocationX, (double)LstCollectingPoints[j].LocationY);               }
        //    }
        //}
        ...
        //פונקציה שמחשבת לכל עובד ולכל נקודה מה המיקום שלה
        //אפשר גם לעדכן ישר במסד נתונים
        //public void CalculateLocationToEmployeeAndPoint()
        //{
        //    foreach (EmployeeDto employee in LstEmployees)
        //    {
        //        // קבל את המיקום של הכתובת של העובד
        //        Tuple<double, double> location = MyGoogleMapsApi.GetLocation(employee.Address);

        //        employee.LocationX = (decimal)location.Item1;
        //        employee.LocationY = (decimal)location.Item2;
        //    }

        //    // קבל את מיקום כל נקודה
        //    foreach (CollectingPointDto point in LstCollectingPoints)
        //    {
        //        // קבל את המיקום של הכתובת של הנקודה
        //        Tuple<double, double> location = MyGoogleMapsApi.GetLocation(point.Address);

        //        point.LocationX = (decimal)location.Item1;
        //        point.LocationY = (decimal)location.Item2;
        //    }
        ...
        //קביעה לכל עובד מי הנקודות שלו
        //אפשר בבנית הגרף לקבוע לכל עובד את הנקודות שבגרף שלו
        //public void CollectingPointToEmployees()//רץ ועובד(לא בדקתי מידי לעומק)י
        //{
        //    //int[] max=new int[LstEmployees.Count];//שמירת המרחק של החבילה הכי קרובה
        //    //CollectingPointDto[] maxCol=new CollectingPointDto[LstEmployees.Count];
        //    for (int i = 0; i < LstEmployees.Count; i++)
        //    {
        //        for (int j = 0; j < Graph.Nodes.Count; j++)
        //        {
        //            if (inRadiousLocation(LstEmployees[i], Graph.Nodes[j].Value) == true)
        //                LstEmployees[i].colectingPointsOfEmployees.Add(Graph.Nodes[j]);
        //            //decimal distance = MyGoogleMapsApi.googleMapsDistance(LstEmployees[i].Address, LstCollectingPoints[j].Address);
        //            //decimal distance = GoogleMapsApigoogleMapsDistance(LstEmployees[i].LocationX,
        //            //        LstEmployees[i].LocationY,
        //            //        LstCollectingPoints[j].LocationX,
        //            //        LstCollectingPoints[j].LocationY);
        //            //if (max[i] < distance)
        //            //{
        //            //    max[i] = (int)distance;
        //            //    maxCol[i] = LstCollectingPoints[j];
        //            //}
        //        }
        //    }
        //} 

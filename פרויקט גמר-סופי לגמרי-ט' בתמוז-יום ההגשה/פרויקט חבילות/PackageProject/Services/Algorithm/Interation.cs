...
namespace Services.Algoritm
{
    public class Interation
    {
        ...
        public PriorityQueue<EmployeeTime> PriorityQueueEmployeeTime { get; set; }
        public  DistanceType[,] DistansesCol { get; set; }//מטריצת מרחקים בין נקודות איסוף

      

        public Interation(DateTime open,DateTime close,List<EmployeeDto> lste, WeightedGraph<CollectingPointDto> graph, DistanceType[,] distancesCol)
        {
            DistansesCol = distancesCol;
            Graph = graph;
            LstEmployees = lste;
            Options = new Dictionary<EmployeeDto, OptionToEmployee>();
            PriorityQueueEmployeeTime = new PriorityQueue<EmployeeTime>(Comparer<EmployeeTime>
                .Create((x, y) => x.ArrivalTime.CompareTo(y.ArrivalTime)));

            initOptions(open);
           
            CalculateVerticesToVisitToEmployee(open, close);
            MovingEmployeesAndPackagesInGraph();
        }
        ...
        public void CalculateVerticesToVisitToEmployee(DateTime beginTime, DateTime endTime)
        {
               ...
               employee= LstEmployees[i];
                ...
                while (currentTime< endTime && countColOfEmployee > 0)
                {
                    ...
                    if (arrivalTime.CompareTo(endTime) < 0)
                    {
                        ...
                        PriorityQueueEmployeeTime.Enqueue(new EmployeeTime() { Employee=employee,ArrivalTime=arrivalTime });
            ...
            //PriorityQueueEmployeeTime.PrintSorted();
        ...
        public void MovingEmployeesAndPackagesInGraph()
        {
            EmployeeDto employee;
            ...
            while(PriorityQueueEmployeeTime.Count>0)//
            {
                //מחזירים מי העובד שהזמן הנוכחי שלו+ הזמן לנקודה הבאה=לזמן הכי קרוב לזמן העכשוי
                
                 employee = PriorityQueueEmployeeTime.Dequeue().Employee;//////////////////
        ...
        public int getNextEmployeeToMove()
        {
        ...
        public void decidingWhichPackagesToDownloadAndWhichNotTo(EmployeeDto employee)
        {
            ...
            foreach (var packageInEmployee in employee.Packages)
            {
                ...
                if(packageInEmployee.package.whereToGetOff==employee.CurrentLocation)
                {
                    ...
                    employee.CurrentLocation.Value.PackagesInCollectingPoint.Add(new PackageInCollectingPoint(packageInEmployee.package,true));
            ...
            foreach (var packageInColPoint in employee.CurrentLocation.Value.PackagesInCollectingPoint)
            {
                ...
                double min = DistansesCol[employee.CurrentLocation.Value.indexDistance, packageInColPoint.package.Destination.Value.indexDistance].kilometer;   //MainAlgorithm.googleMapsDistance(employee.CurrentLocation.Value,packageInColPoint.package.Destination.Value).kilometer;
                ...
                {
                    distance = DistansesCol[colPoint.Value.indexDistance, packageInColPoint.package.Destination.Value.indexDistance].kilometer;                //MainAlgorithm.googleMapsDistance(colPoint.Value, packageInColPoint.package.Destination.Value).kilometer;

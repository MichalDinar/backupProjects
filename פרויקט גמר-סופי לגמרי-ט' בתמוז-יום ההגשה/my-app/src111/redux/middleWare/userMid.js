...
export const middlewareUSERFunctions = ({ dispatch, getState }) => next => action => {
    debugger
    if (action.type == 'GET_USER_BY_NAME_AND_PASSWORD') {
        const getUSERbyNamePassord = async () => {
            debugger
            let name = action.payload.name
            let password = action.payload.password
            action.payload = null
            action.payload = await (await axios.get(`https://localhost:7182/api/User/SignIn?mail=${name}&password=${password}`)).data
           debugger
           
            return next(action)
        }
        getUSERbyNamePassord()
    }
